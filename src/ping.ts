 ...
 */
/**
 * Globalping measurement orchestration and probe catalog.
 * Ported from one-ip (AGPL-3.0) `public/worker/ping.js` (AGPL-3.0).
 */
...
export async function startPing(input) {
  ...
  if (input.regions) {
    ...
    if (!Array.isArray(input.nodes) || !input.nodes.length || input.nodes.length > 50)
      throw new HttpError(400, "单批请选择 1–50 个地区");
    const catalog = input.nodes.some((id) => typeof id === "string" && id.includes(":"))
      ? await pingNodes()
      : nodes;
    const selected = [...new Set(input.nodes)].map((id) => catalog.find((node) => node.id === id));
    if (selected.some((node) => !node)) throw new HttpError(400, "无效的探测地区");
...
export async function pingResult(id) {
  if (!/^[a-zA-Z0-9_-]{8,80}$/.test(id)) throw new HttpError(400, "无效的测量 ID");
...
export async function pingNodes() {
  const probes = await upstream("https://api.globalping.io/v1/probes", {
    cacheTtl: 300,
    maxBytes: 8_000_000,
  });
  ...
  for (const probe of probes) {
    ...
    if (
      ...
      (location.country === "CN" ? chinaCarrier : majorCloud).test(location.network)
    ) {
  ...
  for (const provider of providers.values()) {
    ...
    if (
      ...
      (provider.count === city.preferredProbes && provider.asn < city.preferredAsn)
    ) {
