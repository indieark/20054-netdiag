 ...
 */
/**
 * Cloud vendor status adapters (AWS, GCP, Azure, Aliyun, Tencent, Oracle...).
 * Ported from one-ip (AGPL-3.0) `public/worker/cloud-status.js` (AGPL-3.0).
 */
...
export function parseDmit(data) {
  ...
  if (!states[data?.status] || !Array.isArray(data.services)) throw unavailable();
  return {
...
export function parseGoogleCloud(data) {
  ...
  return summary(
    active.some((item) => item.severity === "high") ? "major" : active.length ? "minor" : "none",
...
export function parseAws(data) {
  ...
  return summary(
    active.some((item) => String(item.status) === "3") ? "major" : active.length ? "minor" : "none",
    active.length ? "存在服务故障" : "正常运行",
    active.map((item) => ({
      id: item.arn,
...
export function parseBandwagon(html) {
  if (!html.includes("<title>BandwagonHost Status</title>")) throw unavailable();
  const label = html
  ...
  if (label === "Operational" && description === "All systems operational")
    return summary(
      "none",
      description,
      /class="empty">No incidents in the last \d+ days\.<\/p>/.test(html) ? [] : undefined,
    );
  if (/maintenance/i.test(label)) return summary("maintenance", description);
  if (/outage|disruption|degraded|incident/i.test(label)) return summary("minor", description);
  throw unavailable();
}
...
export function parseAliyun(data) {
  if (data?.success !== true || data.code !== 200 || !Array.isArray(data.data)) throw unavailable();
  if (
    data.data.some(
      (item) =>
        item?.id == null ||
  ...
  return summary(
    active.some((item) => item.eventType === "ALARM") ? "major" : active.length ? "minor" : "none",
...
export function parseTencentProducts(data, region) {
  ...
  if (
    !products.length ||
    products.some((product) => !["NORMAL", "NOTIFY", "ABNORMAL"].includes(product?.CurrentStatus))
  )
    throw unavailable();
  return products
...
async function tencentStatus(url) {
  const [regionData, bannerData] = await Promise.all([
    upstream(url),
    upstream("https://status.tencentcloud.com/v1/api/status/DescribeHappening?BelongSite=1"),
  ]);
...
export function parseAzure(html) {
  ...
  if (active.some((row) => !plainText(row) || !/<td\b/.test(row))) throw unavailable();
  return summary(
...
export async function getCloudStatus(service) {
  ...
  if (service.id === "azure") return parseAzure(await sourceText(service.url, 8_000_000));
  if (service.id === "dmit") return parseDmit(await upstream(service.url));
  if (service.id === "bandwagonhost") return parseBandwagon(await sourceText(service.url));
  if (service.id === "google-cloud") return parseGoogleCloud(await upstream(service.url));
  if (service.id === "aws") return parseAws(JSON.parse(await sourceText(service.url)));
